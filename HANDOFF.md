...
## Open / next
0. Still to do once Piyush is signed in again: request indexing for `/weddings`, `/cakes/designer`, `/consultation`, `/gallery`; paste the Business Profile share link into CMS › Studio settings and review its pending Google update; create the GA4 web stream, paste the `G-` ID into the CMS and mark `generate_lead` as a key event; Meta Pixel + domain verification (Piyush must log in to Facebook himself); fix the Ads copy and launch only with his budget OK.
