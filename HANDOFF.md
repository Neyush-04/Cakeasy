...
## Open / next
0. Needs Piyush: (a) complete the Firebase Blaze upgrade, then Claude creates the Storage bucket (choose a no-cost US location) and deploys `storage.rules`; (b) GA4: once the first real enquiry arrives, mark `generate_lead` as a key event (Admin › Events), and optionally link GA4 ↔ Google Ads ↔ Search Console; (c) Meta Pixel + domain verification need a browser signed in to Meta as cakeasy94@gmail.com (install Claude in Chrome in that Chrome profile, or log in in the built-in browser); (d) confirm the GBP "review your info" prompt; (e) Ads: pick images, budget and launch himself (or tell Claude a budget).
