export function resolveCakeImage(url: string | undefined | null): string {
  ...
  // Real local assets served from /public (e.g. /gallery/1/img1.jpg) resolve as-is
  if (trimmedUrl.startsWith('/gallery/') || trimmedUrl.startsWith('/assets/') || trimmedUrl.startsWith('/catalog/')) {
    return trimmedUrl;
  }

  // Fallback for any other unrecognized local /src/assets paths or relative URLs
