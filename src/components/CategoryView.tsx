...
export default function CategoryView({ config, setCurrentTab }: CategoryViewProps) {
  return (
    <div className="space-y-20 pb-20 animate-fadeIn">
      <section className="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:items-center">
        <div className="space-y-7 lg:col-span-5">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            <a href={whatsappUrl()} onClick={() => trackWhatsAppClick('category')} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-full border border-[#F0B7C9] px-6 py-3.5 text-xs font-bold uppercase tracking-wider text-[#D63384] hover:bg-[#FFF5F8]"><MessageCircle className="h-4 w-4" /> Chat on WhatsApp</a>
          </div>
        </div>
      ...
      <section className="space-y-8"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><span className="text-xs font-bold uppercase tracking-[0.2em] text-[#D63384]">Curated Cakeasy work</span><h2 className="mt-2 font-serif text-3xl font-bold text-[#251B21]">Inspiration for your brief</h2></div><Link to="/gallery" className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-[#D63384]">View the full archive <ArrowRight className="h-4 w-4" /></Link></div><div className={`grid grid-cols-1 gap-5 ${config.images.length > 3 ? 'md:grid-cols-3' : 'sm:grid-cols-3'}`}>{config.images.map((image, index) => <article key={image} className={`group overflow-hidden rounded-3xl border border-neutral-100 bg-white ${index === 0 && config.images.length > 3 ? 'md:row-span-2' : ''}`}><div className={`${index === 0 && config.images.length > 3 ? 'aspect-[3/4]' : 'aspect-square'} overflow-hidden bg-[#F7F2EF]`}><img src={image} alt={`${config.navLabel} inspiration ${index + 1}`} className="h-full w-full object-cover transition duration-700 group-hover:scale-105" /></div><div className="p-4"><p className="text-xs font-bold uppercase tracking-wider text-[#D63384]">{config.isSmall ? 'Small format' : index === 0 ? 'Signature direction' : 'Design reference'}</p><p className="mt-1 text-sm font-semibold text-[#43242F]">{config.styles[index % config.styles.length]}</p></div></article>)}</div></section>

      <section className="rounded-[32px] bg-[#251B21] p-8 text-white sm:p-12"><div className="grid grid-cols-1 gap-8 lg:grid-cols-12 lg:items-center"><div className="lg:col-span-8"><Sparkles className="h-6 w-6 text-[#F5C178]" /><h2 className="mt-4 font-serif text-3xl font-bold">Tell us what you are celebrating.</h2><p className="mt-3 max-w-2xl text-sm leading-7 text-white/65">We will help turn it into a cake that feels truly yours - with the right scale, flavour, finish and delivery plan.</p></div><Link to="/consultation" className="inline-flex items-center justify-center gap-2 rounded-full bg-[#D63384] px-6 py-3.5 text-xs font-bold uppercase tracking-wider text-white hover:bg-[#B02266] lg:col-span-4">Start your cake consultation <ArrowRight className="h-4 w-4" /></Link></div></section>
    </div>
  );
}
